...
class BondGraph(nx.Graph):
    ...
    def is_fully_percolated(self):
        end_nodes = {node for node in self.nodes if node[1] == self.size-1}
        if len(self.wet_nodes.intersection(end_nodes)) == 0:
            return False
        else:
            return True
...
class HexGraph(nx.Graph):
    ...
    def gen_edges(n, p):
        """
        Generate edges between nodes on the nxn hexagonal grid with probability p
        """
